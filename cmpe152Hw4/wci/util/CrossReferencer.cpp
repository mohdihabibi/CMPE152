                                  ...
                                  vector<TypeSpec *>& record_types) const
{
    ...
    switch ((DefinitionImpl) defn)
    {
        case DF_CONSTANT:
        {
            ...
            bool need_quotes =    (data_value->type == CHAR)
                               || (data_value->type == STRING);
            cout << INDENT << "Value = ";
            if (need_quotes) cout << "'";
            cout << data_value->display();
            if (need_quotes) cout << "'";
            cout << endl;
        ...
        case DF_ENUMERATION_CONSTANT:
        {
            EntryValue *entry_value =
                entry->get_attribute((SymTabKey) CONSTANT_VALUE);
            DataValue *data_value = entry_value->value;
            cout << INDENT << "Value = " << data_value->display() << endl;
    ...
    switch ((TypeFormImpl) form)
    {
        case TF_ENUMERATION:
        {
            TypeValue *type_value =
                typespec->get_attribute(
                                        (TypeKey) ENUMERATION_CONSTANTS);
            vector<SymTabEntry *> constant_ids = type_value->v;
        ...
        case TF_SUBRANGE:
        {
            ...
            TypeSpec *base_typespec = type_value->typespec;
        ...
        case TF_ARRAY:
        {
            ...
            TypeSpec *element_type = type_value->typespec;

            type_value =
                typespec->get_attribute((TypeKey) ARRAY_ELEMENT_COUNT);
            int count = type_value->value->i;
...
string CrossReferencer::print_string(DataValue *data_value) const
{
    return data_value->type == STRING
            ? "'" + data_value->s + "'"
            : data_value->display();
}
